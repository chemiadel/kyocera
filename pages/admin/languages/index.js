			...
			<section className="admin">
							...
							<div className="admin-body">
								<div className="table">
									<table>
										...
										<tbody>
											{loading ? <tr><td>Loading...</td><td /><td /></tr> : data.findLangs.items.map((lang) => (
												<tr key={lang.id}>
													<td>{lang.title}</td>
													<td>{lang.name}</td>
													<td>{lang.statement}</td>
													<td>
														<div className="actions">
															<button className="edit" type="button" onClick={() => window.location.href = `/admin/languages/update/${lang.id}`}>
				...
				>
					<div className="modal-content">
						...
						<form onSubmit={(e) => _onDelete(e)}>
							<p>Do you want to delete this? Once you delete this, you will not able to undo this action.</p>

							<button type="submit" className="btn btn-danger">Delete</button>
							<button type="button" onClick={() => onCloseModal(setDeleteModalIsOpen)} className="btn btn-primary">Cancel</button>
						</form>
					</div>
				</Modal>
			</section>
