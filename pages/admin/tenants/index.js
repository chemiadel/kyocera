...
const DELETE_AD_TENANT = gql`
	mutation deleteAdTenant($id: ID!) {
		deleteAdTenant(input: { id: $id }) {
			id
		}
	}
`;
...
const AdminLanguages = () => {
	...
	const { data, loading } = useQuery(FIND_AD_TENANTS);

	const [deleteTenant, state] = useMutation(DELETE_AD_TENANT);
									...
									<table>
										<thead>
											<tr>
												...
												<th>Country Name</th>
												<th>Azure Client</th>
												<th>Azure Authority</th>
												<th></th>
											</tr>
										</thead>
										<tbody>
											{loading ? <tr><td>Loading...</td><td /><td /></tr> : data.findAdTenants.items.map((tenant) => (
												<tr key={tenant.id}>
													<td>{tenant.country_code}</td>
													<td>{tenant.country_name}</td>
													<td>{tenant.cid}</td>
													<td>{tenant.auth}</td>
													<td>
														<div className="actions">
															
															<button className="edit" type="button" onClick={() => window.location.href = `/admin/tenants/update/${tenant.id}`}>
