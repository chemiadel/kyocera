...
// Modals
import CreateProfession from './create';
import UpdateProfession from './update';
import DeleteProfession from './delete';

const ProfessionsList = ({ activeProfession, activeIndustry, onSelect, role }) => {
	// State
	...
	const { loading, data } = useQuery(
		...
		{
			variables: {
				industryId: activeIndustry ?? 0,
			},
		},
	);

	// Methods
	const openCreateProfessionModal = (e) => {
		e.preventDefault();
	...
	const openUpdateProfessionModal = (e, id) => {
		e.preventDefault();
	...
	const openDeleteProfessionModal = (e, id) => {
		e.preventDefault();
		...
		<div className="radio is-have-actions">
			{(role === 'ADMIN' ) && (
				<div className="actions">
					<button className="edit" type="button" onClick={(e) => openUpdateProfessionModal(e, id)}>
					...
					<button className="delete" type="button" onClick={(e) => openDeleteProfessionModal(e, id)}>
	...
	const RenderList = () => {
		...
		if (data.findProfessions.items.length === 0) {
			return (
				<p style={{ fontSize: 16 }}>
					...
					{(role === 'ADMIN' || role === 'EDITOR') && (
						<>
							...
							<a
								href="#"
								style={{ color: '#0a9bcd' }}
								onClick={(e) => openCreateProfessionModal(e)}
							>
								Create One
							</a>
						</>
		...
		return (
			<>
				{data.findProfessions.items.map(({ id, name }) => (
	...
	return (
		<>
			...
			{(role === 'ADMIN') && (
				<>
					<CreateProfession industryId={activeIndustry ?? 0} />

					<UpdateProfession industryId={activeIndustry ?? 0} id={selectedProfessionId} />
					
					<DeleteProfession industryId={activeIndustry ?? 0} id={selectedProfessionId} />
