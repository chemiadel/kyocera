...
const UPDATE_NEED_MUTATION = gql`
	mutation updateNeed($id: ID!, $title: String!, $content: String!, $fileArray: String) {
		updateNeed(input: { id: $id, title: $title, content: $content, fileArray: $fileArray }) {
			title
			content
			fileArray
		}
...
const UpdateNeed = ({ id, industryId, professionId, languageId, title: propsTitle, content: propsContent, selectedFiles: propsSelectedFiles, onClose }) => {
	// State
	...
	// Context
	const { updateNeedModalIsOpen } = useGlobalState();
	const dispatch = useDispatchGlobalState();
	
	// GraphQL
	const [updateNeed, state] = useMutation(
	...
	return (
		<Modal title="Update" isOpen={updateNeedModalIsOpen} onClose={() => onCloseModal()}>
