...
const CREATE_NEED_MUTATION = gql`
	mutation createNeed($title: String!, $content: String!, $painId: ID!, $position: Int!, $fileArray: String) {
		createNeed(input: { title: $title, content: $content, painId: $painId, position: $position, fileArray: $fileArray }) {
			title
			content
			file
			fileName
...
const CreateNeed = ({ industryId, professionId, painId, languageId, position }) => {
	// State
	...
	// Context
	const { createNeedModalIsOpen } = useGlobalState();
	const dispatch = useDispatchGlobalState();
	
	// GraphQL
	const [createNeed, state] = useMutation(
	...
	return (
		<Modal title="Create new" isOpen={createNeedModalIsOpen} onClose={() => onCloseModal()}>
