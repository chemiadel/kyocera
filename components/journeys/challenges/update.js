...
const UPDATE_CHALLENGE_MUTATION = gql`
	mutation updateChallenge($id: ID!, $title: String!, $content: String!, $fileArray: String) {
		updateChallenge(input: { id: $id, title: $title, content: $content, fileArray: $fileArray }) {
			title
			content
			fileArray
		}
...
const UpdateCasesCard = ({ id, industryId, professionId, languageId, title: propsTitle, content: propsContent, selectedFiles: propsSelectedFiles, onClose }) => {
	...
	// Context
	const { updateChallengeModalIsOpen } = useGlobalState();
	const dispatch = useDispatchGlobalState();
	
	// GraphQL
	const [updateChallenge, state] = useMutation(
	...
	return (
		<Modal title="Update" isOpen={updateChallengeModalIsOpen} onClose={() => onCloseModal()}>
