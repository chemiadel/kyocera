...
const CREATE_PAIN_MUTATION = gql`
	mutation createPain($title: String!, $content: String!, $challengeId: ID!, $position: Int!, $fileArray: String) {
		createPain(input: { title: $title, content: $content, challengeId: $challengeId, position: $position, fileArray: $fileArray }) {
			title
			content
			fileArray
		}
...
const CreatePain = ({ industryId, professionId, challengeId, languageId, position }) => {
	// State
	...
	// Context
	const { createPainModalIsOpen } = useGlobalState();
	const dispatch = useDispatchGlobalState();
	
	// GraphQL
	const [createPain, state] = useMutation(
		CREATE_PAIN_MUTATION,
		{
			refetchQueries: [
				{
	...
	return (
		<Modal title="Create new" isOpen={createPainModalIsOpen} onClose={() => onCloseModal()}>
