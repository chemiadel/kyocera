...
const UPDATE_CASES_CARD = gql`
	mutation updateCaseCard($id: ID!, $title: String!, $content: String!, $fileArray: String) {
		updateCaseCard(input: { id: $id, title: $title, content: $content, fileArray: $fileArray }) {
			id
			title,
			content
			position
...
const UpdateCasesCard = ({ id, industryId, title: propsTitle, content: propsContent, selectedFiles: propsSelectedFiles, onClose, onUpdate }) => {
	// State
	...
	// Context
	const { updateCasesCardModalIsOpen } = useGlobalState();
	const dispatch = useDispatchGlobalState();
	
	// GraphQL
	const [updateCasesCard, state] = useMutation(
		UPDATE_CASES_CARD,
		{
			refetchQueries: [
				{
	...
	const onSubmit = async e => {
		...
		try {
			...
			return onUpdate(result.data?.updateCaseCard);
		} catch (e) {
			console.error(e);
			setLoading(false);
			setError(true);
	...
	return (
		<Modal title="Update" isOpen={updateCasesCardModalIsOpen} onClose={() => onCloseModal()}>
			...
			<form onSubmit={(e) => onSubmit(e)}>
				...
				<ul class="list-files-modal">
					{initialSelectedFiles && initialSelectedFiles.map((file, key) => (
						<li key={key}>
							{file.name}
