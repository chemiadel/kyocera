...
const UPDATE_SALES_CARD = gql`
	mutation updateSaleCard($id: ID!, $title: String!, $content: String!, $fileArray: String) {
		updateSaleCard(input: { id: $id, title: $title, content: $content, fileArray: $fileArray }) {
			id
			title,
			content
			position
...
const UpdateSalesCard = ({ id, industryId, title: propsTitle, content: propsContent, selectedFiles: propsSelectedFiles, onClose, onUpdate }) => {
	// State
	...
	// Context
	const { updateSalesCardModalIsOpen } = useGlobalState();
	const dispatch = useDispatchGlobalState();
	
	// GraphQL
	const [updateSalesCard, state] = useMutation(
		UPDATE_SALES_CARD,
		{
			refetchQueries: [
				{
	...
	const onSubmit = async e => {
		...
		try {
			...
			setLoading(false);
			onUpdate(result.data?.updateSaleCard);
		} catch (e) {
			console.error(e);
			setLoading(false);
			setError(true);
	...
	return (
		<Modal title="Update" isOpen={updateSalesCardModalIsOpen} onClose={() => onCloseModal()}>
...
export default UpdateSalesCard;